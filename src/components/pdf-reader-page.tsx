...
/* 自渲染 canvas + 覆盖层。
   关键：整页（画布 + 术语框 + 段落点）全部用同一个 viewport（同一 scale），
   所以覆盖层坐标 = CSS px = 画布显示尺寸，绝不错位。
   位置提取：getTextContent 的 transform[4]/[5] 是基线原点，transform[3] 是字号，
   用 styles.ascent 上推到字形顶部，宽 = it.width * scale。这我在 node 渲染里验证过是对齐的。 */

interface Popover { x: number; y: number; term: Term; }
interface TermBox { x: number; y: number; w: number; h: number; term: Term; }
interface ParaDot { x: number; y: number; text: string; }
...
interface TextItem { x: number; y: number; w: number; h: number; fs: number; baseline: number; text: string; }

// 读取页文本 + 每项真实 CSS px 框
async function getPageItems(page: any, vp: any): Promise<TextItem[]> {
...
function detectParagraphs(items: TextItem[], pageW: number): ParaDot[] {
  const XGAP = 40;
...
function matchTerms(items: TextItem[], terms: Term[]): TermBox[] {
...
export const ReaderPage = memo(function ReaderPage({
  pageNum, width, doc, terms, onSeg, onTerm,
}: {
  pageNum: number;
  width: number;
  ...
  const [data, setData] = useState<{ w: number; h: number; dots: ParaDot[]; terms: TermBox[] } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const wPx = Math.round(vp.width * dpr);
        const hPx = Math.round(vp.height * dpr);
        canvas.width = wPx;
        canvas.height = hPx;
        ...
        off.width = wPx;
        off.height = hPx;
        await page.render({ canvasContext: off.getContext("2d")!, viewport: { ...vp, scale: vp.scale * dpr, width: wPx, height: hPx } as never }).promise;
        if (cancelled) return;
        ...
        setData({
          w: vp.width,
          h: vp.height,
          dots: detectParagraphs(items, pageW),
    ...
    <div className="pdf-page" style={{ width }}>
      ...
      {data && (
        <div className="pdf-overlay" style={{ width: data.w, height: data.h }}>
          {data.dots.map((p, i) => (
            <button key={`d${i}`} className="pdf-dot"
              style={{ left: Math.max(6, p.x - 10), top: p.y }}
              onClick={(e) => { e.stopPropagation(); onSeg(p.text); }}
              aria-label="点击段落起点：翻译或提问"
              title="翻译 / 提问这一段"
            />
          ))}
          {data.terms.map((t, i) => (
            <button key={`t${i}`} className="pdf-term"
              style={{ left: t.x, top: t.y, width: t.w, height: t.h, fontSize: Math.max(9, t.h * 0.95) }}
              onClick={(e) => {
                e.stopPropagation();
                onTerm({ x: Math.min(e.clientX, window.innerWidth - 300), y: Math.min(e.clientY, window.innerHeight - 240), term: t.term });
              }}
              aria-label={`术语 ${engName(t.term)} 详情`}
              title={engName(t.term)}
            >{engName(t.term)}</button>
          ))}
        </div>
      )}
    </div>
